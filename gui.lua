...
GUI.screenSource = dxCreateScreenSource(sw, sh)
...
GUI.createElement = function(t, args) 
  ...
  e.destroy = function(self)
    for i, v in pairs(self:getChildren()) do
      GUI.elements[v.id]:destroy()
    end
    GUI.elements[self.id] = nil
    --local isElementsToRender
    --for i, v in pairs(GUI.elements) do
    --  if v then
    --    isElementsToRender = true
    --    break
    --  end
    --end
    --if not isElementsToRender then removeEventHandler("onClientRender", root, GUI.render) GUI.data.render = false end
  end
...
GUI.RGBToHex = function(color) -- https://wiki.multitheftauto.com/wiki/RGBToHex
  if not color or type(color) ~= "table" then return nil end
...
GUI.createTransparentCorner = function(px, size)
  local texture = dxCreateTexture(px, px)
  local pixels = dxGetTexturePixels(texture)
  for x = 0, px - 1 do
    for y = 0, px - 1 do
      if math.sqrt(((px - x)^2 + (px - y)^2)) < px and math.sqrt(((px - x)^2 + (px - y)^2)) > px - size then
        dxSetPixelColor(pixels, x, y, 255, 255, 255, 255)
      end
    end
  end
...
GUI.transparentCorners = {}
GUI.getTransparentCorner = function(px, size)
  if not GUI.transparentCorners[px] then GUI.transparentCorners[px] = {} end
  if not GUI.transparentCorners[px][size] then GUI.transparentCorners[px][size] = GUI.createTransparentCorner(px, size) end
  return GUI.transparentCorners[px][size]
end

GUI.createInvertCorner = function(px)
  local texture = dxCreateTexture(px, px)
  local pixels = dxGetTexturePixels(texture)
  for x = 0, px - 1 do
    for y = 0, px - 1 do
      if not (math.sqrt(((px - x)^2 + (px - y)^2)) < px) then
        dxSetPixelColor(pixels, x, y, 255, 255, 255, 255)
      end
    end
  end
...
GUI.tex = {}
GUI.tex.corner = GUI.createCorner(math.floor(256 * (sw / 1920)) % 2 == 0 and math.floor(256 * (sw / 1920)) or math.floor(256 * (sw / 1920)) + 1)
GUI.tex.invertCorner = GUI.createInvertCorner(math.floor(256 * (sw / 1920)) % 2 == 0 and math.floor(256 * (sw / 1920)) or math.floor(256 * (sw / 1920)) + 1)
GUI.tex.check = dxCreateTexture(GUI.C.assets .. "img/check.png")
...
GUI.drawCorneredRectangle = function(x, y, width, height, br, color, postGUI, isNotLimit)
  width = width or height * 2
  height = height or width / 2
  br = isNotLimit and br or math.min(br, math.min(width / 2, height / 2))
...
GUI.render = function()
  ...
  for id = 1, GUI.data.lastID do
    local element = GUI.elements[id]
    if GUI.isElement(element) and element.parent and not element.parent.visible then element.visible = element.parent.visible end
    if GUI.isElement(element) and element.visible then
      ...
      if element.type == "rectangle" then
        if element.borderRadius then
          GUI.drawCorneredRectangle(element.position.x, element.position.y, (element.size.width or element.size.height * 2), (element.size.height or element.size.width / 2), element.borderRadius, GUI.tocolor(element.color.background), element.postGUI)
        else
      ...
      elseif element.type == "button" then
        ...
        c = element.disabled and GUI.tocolor(element.color.disabled) or c
        if element.borderRadius then
      ...
      elseif element.type == "smallCheckbox" then
        local x, y, w, h = element.position.x, element.position.y, math.max(element.size.width, element.size.height), math.max(element.size.height, element.size.width)
        element.size.width = w
        element.size.height = h
        local m = element.margin or GUI.C.defaultMargin
        if element.borderRadius then
        ...
        if element.state then
          dxDrawImage(x + m / 2, y + m / 2, w - m, h - m, GUI.tex.check, 0, 0, 0, GUI.tocolor(element.color.primary))
        end
      elseif element.type == "scroll" then
        ...
        GUI.drawCorneredRectangle(math.floor(element.position.x + element.size.width - (element.scrollWidth or GUI.C.defaultScrollWidth)), math.floor(element.position.y), math.floor(element.scrollWidth or GUI.C.defaultScrollWidth), math.floor(element.size.height), (element.borderRadius or 0), GUI.tocolor(element.color.background), element.postGUI)
        GUI.drawCorneredRectangle(math.floor(element.position.x + element.size.width - (element.scrollWidth or GUI.C.defaultScrollWidth)), math.floor(sY), math.floor(element.scrollWidth or GUI.C.defaultScrollWidth), math.floor(sH), (element.borderRadius or 0), sColor, element.postGUI)
      ...
      if element.border then
        local borderColor = GUI.tocolor(element.color.border or element.color.primary)
        if element.borderRadius > 0 then
          local radius = math.min(element.borderRadius, math.min(element.size.width / 2, element.size.height / 2))
          local borderTexture = GUI.getTransparentCorner(radius + element.border, element.border)
          dxDrawImage(element.position.x - element.border, element.position.y - element.border, radius + element.border, radius + element.border, borderTexture, 0, 0, 0, borderColor, element.postGUI)
          dxDrawImage(element.position.x + element.size.width - radius, element.position.y - element.border, radius + element.border, radius + element.border, borderTexture, 90, 0, 0, borderColor, element.postGUI)

          dxDrawImage(element.position.x + element.size.width - radius, element.position.y + element.size.height - radius, radius + element.border, radius + element.border, borderTexture, 180, 0, 0, borderColor, element.postGUI)
          dxDrawImage(element.position.x - element.border, element.position.y + element.size.height - radius, radius + element.border, radius + element.border, borderTexture, 270, 0, 0, borderColor, element.postGUI)


          dxDrawRectangle(element.position.x - element.border, element.position.y + radius, element.border, element.size.height - radius * 2, borderColor)
          dxDrawRectangle(element.position.x + element.size.width, element.position.y + radius, element.border, element.size.height - radius * 2, borderColor)
        
          dxDrawRectangle(element.position.x + radius, element.position.y - element.border, element.size.width - radius * 2, element.border, borderColor)
          dxDrawRectangle(element.position.x + radius, element.position.y + element.size.height, element.size.width - radius * 2, element.border, borderColor)
        else
          dxDrawRectangle(element.position.x - element.border, element.position.y - element.border, element.border, element.size.height + element.border * 2, borderColor)
          dxDrawRectangle(element.position.x + element.size.width, element.position.y - element.border, element.border, element.size.height + element.border * 2, borderColor)
        
          dxDrawRectangle(element.position.x, element.position.y - element.border, element.size.width, element.border, borderColor)
          dxDrawRectangle(element.position.x, element.position.y + element.size.height, element.size.width, element.border, borderColor)
        end
      end
      if GUI.devmode then
        ...
        if GUI.devmodeLine then
          ...
          for i = 1, math.floor(sw / 2 / GUI.devmodeLine) do
            dxDrawLine(sw / 2 + i * GUI.devmodeLine, 0, sw / 2 + i * GUI.devmodeLine, sh, tocolor(i % 3 == 0 and 180 or 0, i % 3 == 1 and 180 or 0, i % 3 == 2 and 180 or 0, 60))
            dxDrawLine(sw / 2 - i * GUI.devmodeLine, 0, sw / 2 - i * GUI.devmodeLine, sh, tocolor(i % 3 == 0 and 180 or 0, i % 3 == 1 and 180 or 0, i % 3 == 2 and 180 or 0, 60))
          end
          for i = 1, math.floor(sw / 2 / GUI.devmodeLine) do
            dxDrawLine(0, sh / 2 + i * GUI.devmodeLine, sw, sh / 2 + i * GUI.devmodeLine, tocolor(i % 3 == 0 and 180 or 0, i % 3 == 1 and 180 or 0, i % 3 == 2 and 180 or 0, 60))
            dxDrawLine(0, sh / 2 - i * GUI.devmodeLine, sw, sh / 2 - i * GUI.devmodeLine, tocolor(i % 3 == 0 and 180 or 0, i % 3 == 1 and 180 or 0, i % 3 == 2 and 180 or 0, 60))
          end
        end
      end
...
GUI.isElement = function(element)
  return type(element) == "table" and element.id
end

GUI.getActiveElement = function()
  for i = GUI.data.lastID, 1, -1 do
...
addEventHandler("onClientClick", root, function(bttn, state, x, y)
  ...
  for i, v in pairs(GUI.elements) do
    if GUI.isElement(v) and v.type == "scroll" then
      v.scrollActive = nil
    end
  end
  ...
  for i = GUI.data.lastID, 1, -1 do
    ...
    if GUI.isElement(element) then
      ...
      if element.type == "scroll" and element.visible and state == "down" then
      ...
      if GUI.isCursorInPosition(realX, realY, element.size.width, element.size.height) and element.visible and not element.disabled then
        if element.onclick and type(element.onclick) == "function" then
          element.onclick(bttn, state)
        end
        if (element.type == "checkbox" or element.type == "smallCheckbox") and state == "down" then
...
GUI.getElementsByType = function(type, onlyVisible)
  local elements = {}
  for i = 1, GUI.data.lastID do
    if GUI.isElement(GUI.elements[i]) then
      if GUI.elements[i].type == type then
        if onlyVisible then
          if GUI.elements[i].visible then
...
addEventHandler("onClientKey", root, function(bttn, isPressed)
  ...
  if not isPressed then
    if GUI.fastBackspaceSetTimer and isTimer(GUI.fastBackspaceSetTimer) then GUI.fastBackspaceSetTimer:destroy() end
    if GUI.fastBackspaceTimer and isTimer(GUI.fastBackspaceTimer) then GUI.fastBackspaceTimer:destroy() end
    return
  end
  ...
  if bttn == "backspace" then
    ...
    if GUI.fastBackspaceSetTimer and isTimer(GUI.fastBackspaceSetTimer) then GUI.fastBackspaceSetTimer:destroy() end
    if GUI.fastBackspaceTimer and isTimer(GUI.fastBackspaceTimer) then GUI.fastBackspaceTimer:destroy() end
    GUI.fastBackspaceSetTimer = setTimer(function()
      GUI.fastBackspaceTimer = setTimer(function()
  ...
  elseif bttn == "tab" then
    local editboxes = GUI.getElementsByType("editbox", true)
    local isActiveGiven
...
image.position = {x = sw / 2 + margin, y = sh / 2 + margin - 64}
image.size = {width = 512 - margin * 2, height = 256 - margin * 2}
image.color.image = color.flat.red
image.rotation = 0

local smallCheckbox = GUI.createElement("smallCheckbox")
smallCheckbox.position = {x = sw / 2 + margin, y = sh / 2 + margin + 192}
smallCheckbox.size.width = 32

local smallCheckbox2 = GUI.createElement("smallCheckbox")
smallCheckbox2.position = {x = sw / 2 + margin + margin + 32, y = sh / 2 + margin + 192}
smallCheckbox2.size.width = 32
smallCheckbox2.state = true
